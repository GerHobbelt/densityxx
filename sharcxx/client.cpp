...
namespace density {
    ...
    FILE *
    client_io_t::check_open_file(const char *file_name, const char *options,
                                 const bool check_overwrite)
    {
    ...
    uint_fast64_t
    client_io_t::reload_input_buffer(const client_io_t *RESTRICT io_in) {
        uint_fast64_t read = (uint_fast64_t)
            fread(input_buffer, sizeof(uint8_t), SHARC_PREFERRED_BUFFER_SIZE, io_in->stream);
        // FROM HERE.
        stream->update_input(stream, input_buffer, read);
 ...
 uint_fast64_t sharc_client_emptyOutputBuffer(density_stream *restrict stream, const sharc_client_io *restrict io_out) {
    uint_fast64_t available = density_stream_output_available_for_use(stream);
    uint_fast64_t written = (uint_fast64_t) fwrite(output_buffer, sizeof(uint8_t), (size_t) available, io_out->stream);
 ...
 void sharc_client_actionRequired(uint_fast64_t *read, uint_fast64_t *written, const sharc_client_io *restrict io_in, const sharc_client_io *restrict io_out, density_stream *restrict stream, const DENSITY_STREAM_STATE streamState, const char *errorMessage) {
    switch (streamState) {
        ...
        case DENSITY_STREAM_STATE_STALL_ON_INPUT:
            *read = sharc_client_reload_input_buffer(stream, io_in);
            break;
 ...
 void sharc_client_compress(sharc_client_io *io_in, sharc_client_io *const io_out, const DENSITY_COMPRESSION_MODE attemptMode, const bool prompting, const bool integrityChecks, const char *inPath, const char *outPath) {
    ...
    while ((read == SHARC_PREFERRED_BUFFER_SIZE) && (streamState = density_stream_compress_continue(stream)))
 ...
 void sharc_client_decompress(sharc_client_io *io_in, sharc_client_io *const io_out, const bool prompting, const char *inPath, const char *outPath) {
    ...
    while ((read == SHARC_PREFERRED_BUFFER_SIZE) && (streamState = density_stream_decompress_continue(stream)))
